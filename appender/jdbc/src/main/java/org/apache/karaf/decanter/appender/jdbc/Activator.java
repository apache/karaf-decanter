    ...
    public void updated(final Dictionary config) throws ConfigurationException {
        ...
        tracker = new ServiceTracker<DataSource, ServiceRegistration>(bundleContext,bundleContext.createFilter("(name=jdbc-appender)"),null){
            ...
            public ServiceRegistration<?> addingService(ServiceReference reference) {
                ...
                boolean UsePoolPreparedStatement = config != null ? (boolean)config.get("UsePoolPreparedStatement") : true;
                int maxPreparedStatements = config != null ? Integer.parseInt((String)config.get("maxPreparedStatements")) : 50;
                ...
                appender = new JdbcAppender(dataSource,UsePoolPreparedStatement,maxPreparedStatements,tableName,columnName1,columnName2);
        ...
        };}catch (InvalidSyntaxException e){logger.error(e.getMessage());}
